...
function convertNumberToString(numberStr) {
  ...
  for (let i = 0; i < numberStr.length; i += 1) {
    ...
    switch (numberStr[i]) {
      ...
      default:
        str =
          'FUCKING LINTER FUCKS MY BRAIN THIS NIGHT!\nFUCK YOU FUCKING LINTER!';
    }
  }
  return str;
}
...
function isContainNumber(num, digit) {
  ...
  const count = 10;
  while (numNum >= 1) {
    if (numNum % count === digit) {
      return true;
    }
    numNum = Math.floor(numNum / count);
  }
  return false;
}
...
function getNearestBigger(number) {
  ...
  for (let i = numStr.length - 2; i >= 0; i -= 1) {
    ...
    if (+numStr[i] < +numStr[i + 1]) {
      count = +numStr[i];
      numStr = [...numStr].filter((item, index) => index < i).join('');
      break;
    }
  }
  arr.sort((a, b) => a - b);
